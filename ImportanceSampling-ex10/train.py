...
from dataset.load_dataset import data_config_CL
...
def main(args):
    ...
    st = time.time()
    num_completed_sets_prev = -1

    for epoch in range(initial_epoch, total_epochs + 1):
        # Define a variable to keep track of completed sets of 40 epochs
        num_completed_sets = epoch // 40
        # Check if a new set of 40 epochs has been completed
        if num_completed_sets > 0 and num_completed_sets > num_completed_sets_prev:
            num_completed_sets_prev = num_completed_sets
            # Define the values to be added for each set of 40 epochs
            add_values = [1, 1, 1, 1, 0]  # Add certain value every 40 epochs
            # Calculate the index to access the add_values list
            add_index = min(num_completed_sets - 1, len(add_values) - 1)
            # Add the corresponding value to args.randaugm_M
            args.randaugm_M += add_values[add_index]
            value_to_add = add_values[add_index]
            train_loader = data_config_CL(args)
            print(f"Epoch {epoch}: Added {value_to_add} to args.randaugm_M. New value: {args.randaugm_M}")
